...
#define GET_MESHLET_IDX(i) meshletIdx = DispatchMeshArgs[i]
#define OUT_IDX(i) 0
#define indices uint meshletIdx, out
...
void main(uint dtid : SV_DispatchThreadID, uint gtid : SV_GroupThreadID, uint gid : SV_GroupID)
{
	...
	uint meshletIdx;
	MSMain(dtid, gtid, gid, payload, moc, verts, meshletIdx, tris);

	if (gtid < MAX_PRIM_COUNT)
	{
		const uint baseAddr = 3 * (MAX_PRIM_COUNT * meshletIdx + gtid);
		const uint baseIdx = MAX_VERT_COUNT * meshletIdx;
